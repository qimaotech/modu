...
**版本**: 1.0 | **来源**: openspec/changes/add-env-var-support

## 目的

支持在配置文件中的 `workspace` 和 `worktree-root` 字段使用环境变量，允许不同开发者在不同机器上使用同一份配置文件。
...
- 实现：`internal/config/config.go`（resolveEnvVars、extractUndefinedVars）
